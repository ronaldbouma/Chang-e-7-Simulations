...
baseline_amount = [10000,50000] #[5000,10000,50000]
...
for i in range(len(frequency)):
    ...
    for k in range(len(baseline_amount)):
        ...
        Vs, Baselines = funcs.Visibility_sampling_weclipse(Positions, model_x, model_I, wavelength[i], \
                                                  orientations=orientations,Noise=[0,noise_sky], system_noise_factor=0.1, bandwidth=bandwidth, antenna_length=5)[:2]
        full_sky, Vs_clean_tot,Vs_cleaning, points_removed, pixels = funcs.Cleaning_2(Vs, Positions, model_x, wavelength[i], floor=1.5*intensities_fossil[i],cycles=50,points_per_cycle=int(NSIDE/4))
        
        filename = os.path.join("output/cleaned/F{}MHz_B{}.csv".format(int(frequency[i]/1e6),len(Baselines[0])))
