...
def kepler(x,u=398600.441): #input state vector and return keplerian elements
    #inputs are the x,y and z position (in km) and then the x,y and z velocity (in m/s) (single array)
    #outputs are the semi major axis (in meters), the eccentricity, the incination (in radians),
    #the longitude of the ascending node (radians), the Argument of periapsis (radians),
    #the true anomaly (radians), and the eccentric and mean anomaly (radians, only used for determining the true anomaly)
    import numpy as np
...
#unperturbed
#takes the data and turns the postions into km instead of meters
x_pos = data[0:,1]/1000
y_pos = data[0:,2]/1000
z_pos = data[0:,3]/1000
...
#perturbed
#only selects one data point for every 50000 data points by default, can change it
...
#plots the important keplerian elements over the full orbit duration
...
#show start, mid and end orbit, 3d plot to see how the orbit changes over the period
...
#sets the length of the three orbits shown
orbit_l = 4000
