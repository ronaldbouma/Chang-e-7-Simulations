...
def state(x,u=4.9048695e3):
    ...
    #if left empty gravitational parameter is that of the moon
    ...
    #outputs the x,y,z position in km, and the velocity in those directions in km/s 
...
def baselines(t_range,ground_state,sat1_state,sat2_state,ground_orien_init,sat1_orien,sat2_orien,sat1_mut=[0,0],sat2_mut=[0,0],full_output=False):
    ...
    #full output also puts the negative versions of the uv plane into the mix
    if full_output==True:
...
def rot_transform(orien,target):
    ...
    if np.linalg.norm(k)==0:
        ...
        #r = Rotation.from_rotvec(beta*khat).as_matrix() dont use, is slow
...
def cart_to_sph(X_hats,lat=False): #convert carthesian directional vector to angles
    x = X_hats[:,0]
    y = X_hats[:,1]
    z = X_hats[:,2]
...
def beachball(Baseline,x_hats,wavelength):
    ...
    D_dot_sig = np.matmul(D,a) #matmul is better than dot (in terms of performance)
...
def reconstruct(Positions, Vs, NPIX, model_x ,wavelength, divide_by_baselines=False,calibration=False, Vs_c=0, bars=False):
    ...
    #divide by baselines divides the end result by the amount of baselines\
        ...
        # for real use should probably divide each direction by number of counts as visisble (addendum: not nececarily true this, some testing should be done to identify best approach)
...
def grid_testing(target,size,Positions,wavelength,resolution=(200,200), source_amount=5, source_strength = 'random',seed=None):
    #testing gridding function
    #to visualize a small sectioon of the sky at target with size
    #inputs are the centre of the section (target, in degrees) and size (in radians)
    #positions are the xyz coordinates of the measuring elelemnts
    Baselines = np.array([Positions[0]-Positions[1],Positions[0]-Positions[2], Positions[1]-Positions[2]])
...
def gaussian_blob(max_intensity,model_x,direction,size_ang,integrated=False):
    ...
    #max_intensity is the intensity at the center, it falls off with sigma=size_ang (radians)
