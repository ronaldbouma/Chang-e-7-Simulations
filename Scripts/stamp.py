...
#using the stamp reconstruction to give results on a point source and PSF\
    #can use both keplerian orbits or orbital paramaters input from tudat file
...
#from dat, use this for overwriting the simulated keplerian data with real tuDat data.
data = np.genfromtxt(os.path.expanduser('~/desktop/thesis/orbit/perturbedtest/perturbedsattest_a2338_s10_i76.dat'),
                     delimiter=',')

data_select_t = np.sort(np.random.randint(0,len(data),baseline_amount))
data_select = data[data_select_t]

data_position = np.append(data_select[:,1:4],-1*data_select[:,1:4],axis=0)

del data

Ds = funcs.baselines(data_select_t, ground_state, sat1_state, sat2_state, ground_orien, sat1_orien, sat2_orien)

Positions = np.zeros((3,2*baseline_amount,3))

for i in range(len(Positions)):
    Positions[i] = np.append(Ds[i+2],-1*Ds[i+2],axis=0)

Positions[0] = data_position
...
uvw2,uv2,psf2,grid2,points2 = funcs.uvw_transform(target, -Positions, wavelength,grid_resolution=300,psf_output=True)
...
plt.figure(2)
plt.imshow(grid+grid2,origin='lower',extent=(points[0][0],points[0][-1],points[1][0],points[1][-1]))
plt.xlabel('u')
plt.ylabel('v')
...
u_array = np.zeros([len(targets),2*3*baseline_amount])
v_array = np.zeros([len(targets),2*3*baseline_amount])
